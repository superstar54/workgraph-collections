...
def add_adsorbate_to_nanoparticle(
    ...
    distances, neighbors = get_distances(
        atoms.positions[index], atoms.positions, mic=True
    )

    neighbor_indices = np.where(distances < neighbor_radius)[0]
    ...
    if len(neighbor_vectors) >= 1:
        surface_normal = np.linalg.norm(np.sum(neighbor_vectors, axis=0))
    else:
        surface_normal = np.array([0, 0, 1])
    adsorbate = adsorbate.copy()
    # Rotation matrix to align the molecule along the surface normal
    axis = np.array([0, 0, 1])
    angle = np.arccos(np.dot(axis, surface_normal))
    rotation_axis = np.cross(axis, surface_normal)
    if np.linalg.norm(rotation_axis) > 0:  # Avoid dividing by zero
        ...
        adsorbate.rotate(angle, v=rotation_axis, center=(0, 0, 0))

    # Translate the molecule along the surface normal
    offset = (
        distance * surface_normal
    )  # Place the CO molecule 2 Å above the surface atom
    # Translate the molecule to the surface atom's position, offset by the normal
    adsorbate.translate(atoms.positions[index] + offset)
    atoms += adsorbate
