...
def relax_workgraph(structure=None, inputs=None, max_iterations=5, volume_threshold=0.1):
    ...
    tree.links.new(prepare_relax_inputs_node.outputs[0], relax_node.inputs['pw.parameters'])
    # -------- inspect relax -----------
    inspect_relax_node = tree.nodes.new(
        inspect_relax, name='inspect_relax', prev_cell_volume='{{prev_cell_volume}}', volume_threshold=volume_threshold
    )
